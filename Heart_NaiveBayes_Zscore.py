...
plt.savefig('plots/Heart_NaiveBayes_zscore_holdout.png')
_, score_holdout = NaiveBayesEstimation(trnX, tstX, trnY, tstY)
plt.savefig('plots/Heart_NaiveBayes_zscore_holdout_estimators.png')
...
for train_index, test_index in skf.split(X, y):
    ...
    plt.savefig('plots/Heart_NaiveBayes_zscore_CrossVal5_#'+str(i)+'.png')
    xvalues, score_crossval[i] = NaiveBayesEstimation(X_train, X_test, y_train, y_test)
    plt.savefig('plots/Heart_NaiveBayes_zscore_CrossVal5_#'+str(i)+'_estimators.png')
    i+=1
...
print('CrossVal best estimator: %s with score %.2f' % (xvalues[score_bestarg % 2], score_best))
...
plt.savefig('plots/Heart_NaiveBayes_zscore_CrossVal5_mean_estimators.png')
plt.show()
